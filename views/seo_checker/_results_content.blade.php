        ...
        <div style="background: rgba(0,0,0,0.03); border-radius: 16px; padding: 20px; border: 1px solid rgba(0,0,0,0.05); position: relative;">
            ...
            @else
                ...
                <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; width: 100%;">
                    ...
                    @if($userRole === 'guest')
                        <a href="{{ route('login') }}" class="btn btn-primary btn-sm rounded-pill px-4">{{ __('messages.seo_login_to_view') }}</a>
                    @elseif($userRole === 'member' && $settings['speed'] === 'premium')
                        <a href="{{ route('billing.plans') }}" class="btn btn-primary btn-sm rounded-pill px-4">{{ __('messages.seo_upgrade_premium') }}</a>
                    @endif
                </div>
            @endif
        </div>
