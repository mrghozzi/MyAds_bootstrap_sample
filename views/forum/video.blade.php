...
<div class="container py-3 video-watch-page">
    <!-- ADS -->
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8 col-xl-8">
            ...
            <div class="video-stage-card shadow-sm mb-3" id="videoStageCard">
                @if($videoFile)
                    <div class="myads-video-player-container" id="playerContainer_{{ $status->id }}">
                        <video id="videoElement_{{ $status->id }}" class="myads-video-element" poster="{{ $topic->image_url ? asset($topic->image_url) : '' }}" preload="metadata" playsinline>
            ...
            <div class="video-author-actions-card shadow-sm mb-3 p-3 rounded-4 bg-white border">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                    <!-- PUBLISHER IDENTITY -->
                    <div class="d-flex align-items-center gap-3">
                        <!-- HEXAGON AVATAR (HEXAGON CLIP-PATH FALLBACK) -->
            ...
            <div class="video-details-card shadow-sm mb-4 p-4 rounded-4 bg-white border">
                <h1 class="video-watch-title h4 fw-bold mb-2">{{ $topic->name }}</h1>
            ...
            <div class="video-comments-card shadow-sm p-4 rounded-4 bg-white border mb-4">
        ...
        <div class="col-lg-4 col-xl-4">
            <div class="suggested-videos-card shadow-sm sticky-top p-3 rounded-4 bg-white border" style="top: 80px; z-index: 10;">
                <h3 class="suggested-card-heading h6 fw-bold mb-3 d-flex align-items-center">
                ...
                <div class="suggested-videos-list">
                    @forelse($suggestedVideos as $sVideo)
                        ...
                        @if($sTopic && (int) $sVideo->s_type !== 4 && (int) $sVideo->s_type !== 14)
                            <a href="{{ route('forum.topic', $sTopic->id) }}" class="suggested-video-item text-decoration-none d-flex gap-2 mb-3 pb-3 border-bottom">
                                <!-- THUMBNAIL BOX -->
                                <div class="suggested-thumb-box flex-shrink-0" style="width: 115px; height: 68px; border-radius: 8px; overflow: hidden; position: relative; background: #111;">
                                    @if($sTopic->image_url)
                                        <img src="{{ asset($sTopic->image_url) }}" alt="{{ $sTopic->name }}" class="suggested-thumb-img w-100 h-100" style="object-fit: cover;" loading="lazy">
                                    @else
                                        <div class="suggested-thumb-placeholder w-100 h-100 d-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, #1d2333, #615dfa);">
                                            <i class="fa fa-play-circle fa-2x text-white-50"></i>
                                        </div>
                                    @endif
                                    <span class="video-play-badge" style="position: absolute; bottom: 4px; right: 4px; background: rgba(0,0,0,0.75); color: #fff; font-size: 10px; padding: 2px 5px; border-radius: 3px;">
                                        <i class="fa fa-play"></i>
                                    </span>
                                </div>
                                ...
                                <div class="suggested-video-info flex-grow-1 min-w-0">
                                    <h4 class="suggested-video-title text-truncate-2 mb-1" style="font-size: 13px; font-weight: 600; color: #222; line-height: 1.35;">
                                        {{ $sTopic->name }}
                                    </h4>
                                    <div class="suggested-video-author text-muted small text-truncate">
                                        <i class="fa fa-user me-1"></i> {{ $sTopic->user?->username }}
                                    </div>
                                    <div class="suggested-video-meta text-muted extra-small mt-1" style="font-size: 11px;">
...
.v-btn:hover { background: rgba(255, 255, 255, 0.2); color: #615dfa; }
.v-btn-badge { width: auto; padding: 2px 8px; border-radius: 12px; font-size: 11px; background: rgba(255, 255, 255, 0.15); }
.v-time-text { color: #e0e0e0; font-size: 12px; font-family: monospace; }
.v-volume-range { width: 55px; accent-color: #615dfa; cursor: pointer; }

.v-action-buttons-wrap {
    display: flex !important; align-items: center !important;
    flex-direction: row !important; gap: 8px !important; flex-wrap: wrap !important;
}
.v-flyout-wrap { position: relative !important; display: inline-flex !important; align-items: center !important; }
.v-action-btn {
    background: #f0f2f5 !important; border: 1px solid #e0e6ed !important;
    color: #4a4d68 !important; font-size: 13px !important; font-weight: 600 !important;
    height: 36px !important; padding: 0 16px !important; border-radius: 18px !important;
    display: inline-flex !important; align-items: center !important; justify-content: center !important;
    cursor: pointer !important; white-space: nowrap !important; width: auto !important; flex: 0 0 auto !important;
}
.v-action-btn:hover { background: #615dfa !important; color: #fff !important; border-color: #615dfa !important; }
.v-action-btn.icon-only { width: 36px !important; height: 36px !important; padding: 0 !important; border-radius: 50% !important; }

.custom-v-flyout {
    position: absolute !important; top: 42px !important; left: 0 !important; min-width: 180px !important;
    background: #ffffff !important; border: 1px solid #e0e6ed !important; border-radius: 12px !important;
    box-shadow: 0 10px 30px rgba(0,0,0,0.18) !important; padding: 8px !important; z-index: 99999 !important;
}
[dir="rtl"] .custom-v-flyout { left: 0 !important; right: auto !important; }
.v-flyout-item {
    padding: 8px 12px; border-radius: 8px; font-size: 13px; font-weight: 600; color: #333;
    cursor: pointer; display: flex; align-items: center; transition: background 0.15s ease;
}
.v-flyout-item:hover { background: rgba(97, 93, 250, 0.12); color: #615dfa; }
.v-speed-item { padding: 6px 10px; font-size: 12px; color: #fff; border-radius: 6px; cursor: pointer; }
.v-speed-item:hover, .v-speed-item.active { background: #615dfa; }
