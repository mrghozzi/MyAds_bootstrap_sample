...
<div class="knowledgebase-page">
    ...
    <div class="card border-0 shadow-sm rounded-4 kb-shell-card no-padding overflow-hidden mb-4">
        <div class="kb-hero">
    ...
    @if($mode === 'list')
        <div class="card border-0 shadow-sm rounded-4 kb-helper-card mb-4" id="kb-new-topic">
            <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 fw-bold fs-5">{{ __('messages.add') }} {{ __('messages.topic') }}</div>
            <div class="card-body p-4">
                <form method="GET" action="{{ route('kb.index', $product->name) }}" class="form">
                    <div class="row gx-2">
                        <div class="col-8 col-md-9"><input type="text" name="st" class="form-control form-control-lg bg-light border-0" value="{{ request('st') }}" placeholder="{{ __('messages.name') }}" required></div>
                        <div class="col-4 col-md-3"><button type="submit" class="btn btn-outline-secondary btn-lg w-100 rounded-pill fw-bold">{{ __('messages.add') }}</button></div>
                    </div>
                </form>
            </div>
        </div>
        ...
        @else
            <div class="kb-topic-grid">
                @foreach($articles as $item)
                    ...
                    @endphp
                    <article class="card border-0 shadow-sm rounded-4 kb-topic-card h-100">
                        ...
                        <div class="kb-topic-card__footer">
                            <a class="btn btn-outline-secondary rounded-pill fw-bold btn-sm px-3" href="{{ route('kb.show', ['name' => $product->name, 'article' => $item->name]) }}"><i class="fa fa-book" aria-hidden="true"></i>&nbsp;{{ __('messages.preview') }}</a>
                        </div>
                    </article>
                @endforeach
            </div>
    ...
    @if($mode === 'create' || $mode === 'edit')
        <div class="kb-editor-layout">
            <div class="card border-0 shadow-sm rounded-4 kb-form-card mb-4">
                <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 fw-bold fs-5">{{ $mode === 'edit' ? ($canManageCurrentArticle ? __('messages.edit_topic') : __('messages.suggest_edit')) : __('messages.add') . ' ' . __('messages.topic') }}</div>
                <div class="card-body p-4">
                    @if(session('kb_error'))
                        <div class="alert alert-danger border-0 shadow-sm rounded-3 fw-bold">{{ session('kb_error') }}</div>
                    @endif
                    <form method="POST" action="{{ route('kb.store') }}">
                        ...
                        @if($mode === 'create')
                            <div class="mb-4">
                                ...
                                <input type="text" name="name" class="form-control form-control-lg bg-light border-0" value="{{ $articleName ?? '' }}" placeholder="{{ __('messages.name') }}" required>
                            </div>
                        @else
                        ...
                        @endif
                        <div class="mb-4">
                            <label class="form-label small fw-bold">{{ __('messages.desc') }}</label>
                            <textarea id="kb-editor" name="txt" class="form-control bg-light border-0" rows="15" required>{{ old('txt', $editorText ?? '') }}</textarea>
                        </div>
                        <div class="row align-items-center mb-4">
                            <div class="col-auto">
                                <img src="{{ route('kb.captcha') }}" id="kb-captcha" class="rounded-3 shadow-sm border" alt="captcha" style="height: 48px; cursor: pointer;">
                            </div>
                            <div class="col">
                                <input type="text" name="capt" class="form-control form-control-lg bg-light border-0" placeholder="Captcha" required>
                            </div>
                        </div>
                        @if($mode === 'create' && auth()->check())
                            <div class="mb-4">
                                <div class="form-check form-switch form-switch-lg">
                                    ...
                                    <label class="form-check-label fw-bold ms-2" for="kb-share-to-community">{{ __('messages.share_to_community') }}</label>
                                </div>
                                <div class="form-text small fw-bold text-muted mt-2"><i class="fa fa-info-circle me-1"></i> {{ __('messages.knowledgebase_share_to_community_hint') }}</div>
                            </div>
                        @endif
            ...
            <div class="card border-0 shadow-sm rounded-4 kb-helper-card mb-4">
                <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 fw-bold fs-5">{{ __('messages.details') }}</div>
                <div class="card-body p-4">
                    <div class="kb-aside-card__meta mb-4">
                        <div class="kb-meta-row"><span>{{ __('messages.seller') }}</span><strong>{{ $owner ? $owner->username : __('messages.unknown') }}</strong></div>
                        <div class="kb-meta-row"><span>{{ __('messages.topics') }}</span><strong>{{ $articleTotal ?? 0 }}</strong></div>
                        <div class="kb-meta-row"><span>{{ __('messages.pending') }}</span><strong>{{ $pendingTotal ?? 0 }}</strong></div>
                    </div>
                    <div class="d-grid gap-2">
                        <a class="btn btn-outline-secondary rounded-pill fw-bold" href="{{ route('store.show', $product->name) }}"><i class="fa fa-shopping-basket me-1"></i> {{ __('messages.preview') }}</a>
                        <a class="btn btn-light border rounded-pill fw-bold" href="{{ route('kb.index', $product->name) }}"><i class="fa fa-list-ul me-1"></i> {{ __('messages.topics') }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    @if($mode === 'show')
        ...
        <div class="kb-topic-layout">
            <div class="card border-0 shadow-sm rounded-4 kb-main-card mb-4">
                <div class="card-body p-4 p-md-5">
                    <div class="kb-main-card__header">
                        <div>
                            <p class="kb-topic-card__label">{{ __('messages.topic') }}</p>
                            <h3 class="fw-bold fs-3 text-dark mb-2">{{ $article->name }}</h3>
            ...
            <div class="card border-0 shadow-sm rounded-4 kb-side-card mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-grid gap-2 mt-4">
                        @auth
                            @if($knowledgebaseCommunityPublishAction && $knowledgebaseCommunityPublishPayload)
                                <form method="POST" action="{{ $knowledgebaseCommunityPublishAction }}" class="mb-0">
                                    @csrf
                                    <input type="hidden" name="store" value="{{ $knowledgebaseCommunityPublishPayload['store'] }}">
                                    <input type="hidden" name="article" value="{{ $knowledgebaseCommunityPublishPayload['article'] }}">
                                    <button class="btn btn-primary w-100 rounded-pill fw-bold" type="submit"><i class="fa fa-bullhorn me-1"></i> {{ __('messages.share_to_community') }}</button>
                                </form>
                            @endif
                        @endauth
                        <a class="btn btn-outline-secondary rounded-pill fw-bold" href="{{ route('kb.edit', ['name' => $product->name, 'article' => $article->name]) }}"><i class="fa fa-pencil-square-o me-1"></i> {{ $canManageCurrentArticle ? __('messages.edit_topic') : __('messages.suggest_edit') }}</a>
                        <a class="btn btn-light border rounded-pill fw-bold" href="{{ route('kb.pending', ['name' => $product->name, 'article' => $article->name]) }}"><i class="fa fa-hourglass-half me-1"></i> {{ __('messages.pending') }}</a>
                        <a class="btn btn-light border rounded-pill fw-bold" href="{{ route('kb.history', ['name' => $product->name, 'article' => $article->name]) }}"><i class="fa fa-history me-1"></i> {{ __('messages.history') }}</a>
                        <a class="btn btn-light border rounded-pill fw-bold bg-white" href="{{ route('store.show', $product->name) }}"><i class="fa fa-shopping-basket me-1"></i> {{ $product->name }}</a>
                    </div>
    ...
    @if($mode === 'pending' || $mode === 'history')
        ...
        <div class="kb-review-layout">
            ...
            <div class="kb-review-sidebar-area">
                <div class="card border-0 shadow-sm rounded-4 kb-main-card mb-4">
                    <div class="card-body p-4 p-md-5">
                        <div class="kb-main-card__header">
                            <div>
                                <p class="kb-topic-card__label text-uppercase text-muted fw-bold small">{{ __('messages.current') }}</p>
                                <h3 class="fw-bold fs-4 text-dark mb-2">{{ $article->name }}</h3>
                                <p class="kb-main-card__subtitle">
                                    #{{ $article->id }}
                                    @if($articleAuthor)
                ...
                <div class="card border-0 shadow-sm rounded-4 kb-side-card kb-review-card__table mb-4">
                    <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 fw-bold fs-5">{{ $mode === 'pending' ? __('messages.pending') : __('messages.history') }}</div>
                    <div class="card-body p-4">
                        @if($entries->isEmpty())
                            <div class="kb-empty-state">{{ __('messages.no_post') }}</div>
                        @else
                            <form method="POST" action="{{ route('kb.approve') }}">
                                ...
                                @if($isAuthorized)
                                    <div class="mt-3">
                                        <button type="submit" class="btn btn-primary w-100 rounded-pill fw-bold" onclick="return confirm('{{ __('messages.aystqwbc') }}')">{{ $mode === 'pending' ? __('messages.replacing') : __('messages.recovery') }}</button>
                                    </div>
                                @endif
                            </form>
                        @endif
                        <div class="d-grid gap-2 mt-4">
                            <a class="btn btn-light border rounded-pill fw-bold" href="{{ route('kb.edit', ['name' => $product->name, 'article' => $article->name]) }}"><i class="fa fa-pencil-square-o me-1"></i> {{ $canManageCurrentArticle ? __('messages.edit_topic') : __('messages.suggest_edit') }}</a>
                            <a class="btn btn-light border rounded-pill fw-bold bg-white" href="{{ route('kb.show', ['name' => $product->name, 'article' => $article->name]) }}"><i class="fa fa-book me-1"></i> {{ __('messages.topic') }}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="kb-review-content-area">
                <div id="ajax-preview-card" class="card border-0 shadow-sm rounded-4 kb-main-card mb-4" style="display: none;">
                    <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 fw-bold fs-5">{{ __('messages.preview') }}</div>
                    <div class="card-body p-4 p-md-5">
                        <div class="kb-main-card__header mb-4">
                             <div>
                                <h3 class="fw-bold fs-4 text-dark mb-0" id="ajax-preview-title">{{ __('messages.preview') }}</h3>
                             </div>
                        </div>
        ...
        function initKB() {
            ...
            if (textarea && typeof Stackedit !== 'undefined') {
                ...
                editorWrapper.innerHTML = `
                    <button type="button" class="btn btn-outline-secondary btn-sm rounded-pill fw-bold" id="open-stackedit">
