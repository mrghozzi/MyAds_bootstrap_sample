@if($deals->count() > 0)
    <table class="custom-ads-table table table-hover">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach($deals as $deal)
                <tr>
                    ...
                    <td><a href="{{ route('ads.custom.deals.show', $deal) }}" class="btn btn-sm btn-outline-primary">{{ __('messages.details') }}</a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
@else
    <div class="custom-ads-card text-center p-4">
        <p class="custom-ads-muted mb-0">{{ __('messages.custom_ads_no_deals') }}</p>
    </div>
@endif
