...
@if(auth()->id() == $activity->uid || auth()->user()->isAdmin())
                        <li><button class="dropdown-item py-2" onclick="postEdit({{ $activity->tp_id }}, 100)"><i class="fa-regular fa-pen-to-square me-2"></i> {{ __('messages.edit') }}</button></li>
                        <li><button class="dropdown-item py-2 text-danger" onclick="deletePost({{ $activity->tp_id }}, 100, '.post{{ $activity->id }}')"><i class="fa-regular fa-trash-can me-2"></i> {{ __('messages.delete') }}</button></li>
                    @endif
                    @include('theme::partials.activity.promotion_link', ['activity' => $activity])
                    <li><button class="dropdown-item py-2" onclick="reportPost({{ $activity->tp_id }}, 100, {{ $activity->related_content->id }})"><i class="fa-regular fa-flag me-2"></i> {{ __('messages.report') }}</button></li>
