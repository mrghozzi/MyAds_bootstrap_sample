        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body" style="display: grid; gap: 12px;">
                ...
                <div style="padding: 16px; border-radius: 18px; background: linear-gradient(135deg, #f8fbff 0%, #eef6ff 100%);">
                    <p class="mb-2 fw-bold text-dark">{{ __('messages.smart_performance') }}</p>
                    <p class="mb-1 text-muted">{{ $smartAd->impressions }} {{ __('messages.smart_impressions_label') }}</p>
                    <p class="mb-0 text-muted">{{ $smartAd->clicks }} {{ __('messages.smart_clicks_label') }}</p>
                </div>
            </div>
        </div>
    </div>
